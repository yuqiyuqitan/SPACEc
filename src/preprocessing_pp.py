...
def pp_read_data(
    ...
    redundant_info=["x1", "y1", "z1", "filename"],
    cell_size="area",
    reg_num="region_num",
    ...
    dftest = pd.concat(
        (
            pd.read_csv(filename, index_col=None, header=0).assign(
    ...
    dftest[metadata_filename] = dftest.filename.apply(
    ...
    if add_unique_label == True:
        ...
        df_regionout["unique_label"] = (
            df_regionout[unique_data[1]]
            + "_"
            + df_regionout[unique_data[1]]
            + "_"
            + "reg"
            + "_"
            + df_regionout[reg_num].astype(str)
        )
        ...
        g = sns.scatterplot(
            data=df_regionout, x=nuc_marker, y=cell_size, hue="unique_label", size=1
        )
        g.set_xscale("log")
        g.set_yscale("log")
        ...
        plt.figure(figsize=(7, 7))
        g = sns.scatterplot(
            data=df_regionout, x=nuc_marker, y=cell_size, hue=reg_num, size=1
        )
        g.set_xscale("log")
        g.set_yscale("log")
        ticks = [0.1, 1, 10, 100, 1000]
    ...
    df_nuc = df_regionout[
        (df_regionout[nuc_marker] > nuc_thres) * df_regionout[cell_size] > size_thres
    ]
