...
def pl_highlighted_dot(df, x_col, y_col, group_col, highlight_group, highlight_color="red", region_col="unique_region", subset_col=None, subset_list=None):
    """
...
def pl_create_pie_charts(data, group_column, count_column, plot_order=None, show_percentages=True, color_dict=None):
    """
...
def annotate(data,names, **kws):
    ...
    ax = plt.gca()
    ax.text(0.5, .8, 'r={:.2f}, p={:.2g}'.format(r, p),transform=ax.transAxes,
           fontsize = 14)
...
def pl_Barycentric_coordinate_projection(w, 
    ...
    plt.scatter(coords[:,0]+jit*np.random.randn(len(coords)),coords[:,1]+jit*np.random.randn(len(coords)),s = 15,alpha = .5, c = cols)
    plt.axis('off')
    plt.show()
...
def pl_catplot_old(df, hue, X = 'X',Y = 'Y', exp='Exp', exps = None, invert_y = False,size = 3,legend = True, palette="bright",figsize = 5,style = 'white', axis = 'on',scatter_kws = {}):
    '''
...
def pl_mono_cluster_spatial(df, sample_col = 'Sample', cluster_col = 'Cell Type', x= 'x', y ='y', color_dict = None, s = 3, alpha = 0.5,figsize = (15,12)):
    for i in df[sample_col].unique():
        ...
        for ct, ax in zip(celltype, axs.ravel()):
            ...
            if color_dict == None:
                sns.scatterplot(x=x, y=y, data=df_tmp, hue=cluster_col, s=s, alpha=alpha, ax=ax)
            else:
                sns.scatterplot(x=x, y=y, data=df_tmp, hue=cluster_col, s=s, alpha=alpha, ax=ax, palette=color_dict)
